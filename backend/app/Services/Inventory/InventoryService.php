...
class InventoryService
{
     ...
     * @return array<int, array<string, mixed>>
     */
    public function replenishFromTask(GardenOwner $owner, Task $task): array
    {
        $task->loadMissing('requiredResources');

        $resource = $task->requiredResources->first();
        $normalized = $resource
            ? $this->normalizeRequirement($resource)
            : [
        ...
        $quantity = round(max(
            (float) ($task->item_quantity ?? 0),
            (float) ($normalized['shortage_quantity'] ?? 0),
            (float) ($normalized['required_quantity'] ?? 0),
            (float) data_get($task->inventory_context, 'replenishment.quantity', 0),
        ), 2);

        if ($quantity <= 0) {
            throw ValidationException::withMessages([
                'task' => ['The replenishment task does not define a restock quantity.'],
            ]);
        }

        $item = $this->matchingItemsQuery(
            $owner,
            $normalized['normalized_name'],
            $normalized['inventory_item_type'],
            $normalized['unit'],
        )
            ->lockForUpdate()
            ->first();

        $quantityBefore = round((float) ($item?->quantity ?? 0), 2);
        $quantityAfter = round($quantityBefore + $quantity, 2);

        if (! $item) {
            $item = InventoryItem::query()->create([
                'garden_owner_id' => $owner->id,
                'name' => $normalized['resource_name'],
                'normalized_name' => $normalized['normalized_name'],
                'quantity' => $quantityAfter,
                'type' => $normalized['inventory_item_type'],
                'inventory_item_type' => $normalized['inventory_item_type'],
        ...
        InventoryUsageLog::query()->create([
            'inventory_item_id' => $item->id,
            'task_id' => $task->id,
            'task_resource_requirement_id' => $resource?->id,
            'garden_owner_id' => $owner->id,
