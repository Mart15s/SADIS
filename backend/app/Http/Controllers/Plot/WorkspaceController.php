...
class WorkspaceController extends Controller
{
    ...
    public function update(
        ...
        $validated = $request->validate([
            ...
            'zones.*.color_hex' => ['nullable', 'regex:/^#[0-9A-Fa-f]{6}$/'],
            'zones.*.archived_at' => ['nullable', 'date'],
            ...
            'plants.*.variety' => ['nullable', 'string', 'max:255'],
            'plants.*.quantity' => ['nullable', 'numeric', 'min:0'],
            'plants.*.occupied_area' => ['nullable', 'numeric', 'min:0'],
            'plants.*.season' => ['nullable', 'string', 'max:40'],
            'plants.*.harvest_date' => ['nullable', 'date'],
            'plants.*.notes' => ['nullable', 'string', 'max:2000'],
            'plants.*.marker_position_x' => ['nullable', 'numeric', 'between:0,1'],
            'plants.*.marker_position_y' => ['nullable', 'numeric', 'between:0,1'],
