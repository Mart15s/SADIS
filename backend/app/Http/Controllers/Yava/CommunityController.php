...
class CommunityController extends Controller
{
    ...
    public function show(Request $request, Community $community, PermissionService $permissions)
    {
        ...
        return response()->json(['data' => $community->load(['memberships.user:id,email'])]);
    }
    ...
    public function members(Request $request, Community $community, PermissionService $permissions)
    {
        ...
        return response()->json(['data' => CommunityMembership::query()->with('user:id,email')->where('community_id', $community->id)->get()]);
    }

    public function acceptInvitation(Request $request, string $code, OtpService $otp)
    {
        $invitation = CommunityInvitation::query()->where('code_hash', hash('sha256', Str::upper($code)))->firstOrFail();
    ...
    public function decideJoin(Request $request, Community $community, CommunityJoinRequest $joinRequest, PermissionService $permissions)
    {
        ...
        DB::transaction(function () use ($joinRequest, $request, $status): void {
            $joinRequest->update(['status' => $status, 'decided_by_user_id' => $request->user()->id, 'decided_at' => now()]);
            if ($status === 'approved') {
                CommunityMembership::query()->updateOrCreate(
                    ['community_id' => $joinRequest->community_id, 'user_id' => $joinRequest->user_id],
    ...
    public function updateMember(Request $request, Community $community, CommunityMembership $membership, PermissionService $permissions)
    {
        ...
        $removesAdmin = $membership->role === 'admin' && $membership->status === 'active'
        ...
        $membership->update($data + (isset($data['status']) && $data['status'] === 'revoked' ? ['revoked_at' => now()] : []));

        return response()->json(['data' => $membership->fresh()]);
    }
