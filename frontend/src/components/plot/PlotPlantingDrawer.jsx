...
export default function PlotPlantingDrawer({ selectedZone, canEdit, busy, onCreatePlant }) {
  ...
  useEffect(() => {
    ...
    api
      .listCatalogPlants(debouncedSearch.trim() ? { q: debouncedSearch.trim() } : {})
      .then((results) => {
        if (!cancelled) {
  ...
  return (
    <>
      <section className="inspector-section workspace-context-card plant-placement-card">
        <div className="list-head">
          <div className="page-stack">
            ...
            <p className="section-copy">
              Select a zone and a catalog plant, then place it with its shared care profile already
              linked.
            </p>
          </div>
          <Button
          ...
          >
            {selectedZone ? 'Add plant to draft' : 'Select a zone first'}
          </Button>
        </div>
        ...
        {selectedZone ? (
          <div className="meta-cluster">
            ...
            <StatRow label="Soil" value={formatSoilType(selectedZone.soil_type)} />
          </div>
        ) : (
          <EmptyStatePanel
            title="Select a zone first"
            description="Select a zone on the plan before placing a plant in it."
            tone="subtle"
          />
        )}
      </section>
      ...
      >
        <DialogHeader
          title={`Add a plant to ${selectedZone?.name ?? 'the selected zone'}`}
          subtitle="Select a reusable catalog plant and place it in the chosen zone. The change stays in the editor draft until the main save."
          titleId="plant-placement-title"
          subtitleId="plant-placement-subtitle"
          onClose={closeDrawer}
          closeLabel="Close plant panel"
        />
        <DialogBody className="plant-flow-body page-stack">
          <div className="inline-note">
            Zone context locked: <strong>{selectedZone?.name ?? 'selected zone'}</strong>.
          </div>

          <section className="page-stack">
            <div className="field">
              <label htmlFor="drawer-catalog-search">Find a catalog plant</label>
              <input
            ...
            {catalogResults.length > 0 ? (
              <div className="plant-flow-catalog-grid">
                {catalogResults.map((catalogPlant) => {
                  const isSelected =
                    String(selectedCatalogPlant?.id ?? '') === String(catalogPlant.id)
                  ...
                  return (
                    ...
                    >
                      <div className="catalog-plant-identity">
                        ...
                        <span className="catalog-plant-canonical">
                          {catalogPlant.source_scientific_name || catalogPlant.canonical_name}
                        </span>
                      </div>

                      <div className="catalog-plant-meta">
                        <span className="badge badge-soft">
                          {formatPlantType(catalogPlant.plant_type) || 'Unknown type'}
                        </span>
                        {catalogPlant.source_family ? (
                          <span className="badge badge-neutral">{catalogPlant.source_family}</span>
                        ) : null}
                      </div>
                      ...
                      <div className="meta-cluster">
                        <StatRow
                          label="Water every"
                          value={formatDayCount(preview?.watering_interval_days)}
                        />
                        <StatRow
                          label="Fertilize every"
                          value={formatDayCount(preview?.fertilizing_interval_days)}
                        />
                        <StatRow label="Planted" value={catalogPlant.usage_count ?? 0} />
                      </div>
                    </button>
                  )
                })}
              ...
              <div className="inline-note">
                No catalog plants match this search. If needed, create a reusable catalog entry
                first.
              </div>
            ) : null}
          </section>
          ...
          {selectedCatalogPlant ? (
            <form id="plant-placement-form" className="page-stack" onSubmit={handleSubmit}>
              <section className="panel page-stack plant-flow-summary">
                <div className="list-head">
                  <div className="page-stack">
                    ...
                    <p className="section-copy">
                      ...
                      {selectedCatalogPlant.source_scientific_name
                        ? ` | ${selectedCatalogPlant.source_scientific_name}`
                        : ''}
                    </p>
                  </div>
                  <div className="meta-cluster">
                    <span className="badge badge-soft">
                      {formatPlantType(selectedCatalogPlant.plant_type) || 'Unknown type'}
                    </span>
                  </div>
                </div>
                ...
                <div className="form-grid plant-flow-instance-grid">
                  ...
                  <div className="field">
                    <label htmlFor="placement-date">Planting date</label>
                    <input
                      id="placement-date"
                      type="date"
                      value={form.plant_date}
                      onChange={(event) =>
                        setForm((current) => ({ ...current, plant_date: event.target.value }))
                      }
                      required
                    />
                  </div>
                  ...
                  <div className="field">
                    <label htmlFor="placement-variety">Variety</label>
                    <input
                      id="placement-variety"
                      value={form.variety}
                      onChange={(event) =>
                        setForm((current) => ({ ...current, variety: event.target.value }))
                      }
                  ...
                  <div className="field">
                    <label htmlFor="placement-area">Occupied area</label>
                    <input
                      id="placement-area"
                      type="number"
                      min="0"
                      step="0.01"
                      value={form.occupied_area}
                      onChange={(event) =>
                        setForm((current) => ({ ...current, occupied_area: event.target.value }))
                      }
                      placeholder="m²"
                    />
                  </div>
                  ...
                  <div className="field">
                    <label htmlFor="placement-season">Season</label>
                    <select
                      id="placement-season"
                      value={form.season}
                      onChange={(event) =>
                    ...
                    >
                      <option value="">Based on planting date</option>
                  ...
                  <div className="field field-span-2">
                    ...
                    <textarea
                      ...
                      onChange={(event) =>
                        setForm((current) => ({ ...current, disease_notes: event.target.value }))
                      }
                      placeholder="Optional notes about this planted crop"
                    />
                  </div>

                  <div className="field field-span-2">
                    <label htmlFor="placement-notes">Planting notes</label>
                    <textarea
                      id="placement-notes"
                      value={form.notes}
                      onChange={(event) =>
              ...
              <section className="panel page-stack">
                <div className="list-head">
                  <div className="page-stack">
                    <h4 className="section-title">Shared care preview</h4>
                    <p className="section-copy">
                      This planted crop will use the shared care profile linked in the catalog. Edit
                      the catalog plant to change it.
                    </p>
                  </div>
                </div>
                ...
                {selectedCare ? (
                  ...
                  <div className="inline-note">
                    This catalog plant does not have a shared care profile yet. Open the catalog
                    plant and add one before planting.
                  </div>
                )}
            ...
            <div className="panel page-stack">
              <EmptyState
                title="Select a catalog plant"
                description="Search the catalog and select a reusable plant to place in the current zone."
              />
            </div>
          )}
        </DialogBody>
