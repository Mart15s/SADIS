...
function PlantInformationCard({ marker, plotId, canEdit, isMobile, onClose, onReset, onReposition }) {
  ...
  return (
    <section className="plot-plant-info-card" role="dialog" aria-label={plotPlanText('plantInformation')}>
      <button type="button" className="plot-plan-close" onClick={onClose} aria-label="Close crop information">×</button>
      ...
      <dl className="plot-plant-info-grid">
        ...
        {plant.plant_date ? <div><dt>Planted</dt><dd>{plant.plant_date}</dd></div> : null}
        {plant.quantity !== null && plant.quantity !== undefined ? <div><dt>Quantity</dt><dd>{plant.quantity}</dd></div> : null}
        {plant.occupied_area ? <div><dt>Area</dt><dd>{plant.occupied_area} m²</dd></div> : null}
        <div><dt>Next task</dt><dd>{task ? `${task.name} · ${task.date}` : plotPlanText('noRecommendedTask')}</dd></div>
      </dl>
      <div className="plot-plant-info-actions">
        ...
        {canEdit ? <Button size="sm" variant="ghost" onClick={() => onReposition(plant.id)}>{isMobile ? 'Move crop marker' : 'Move marker'}</Button> : null}
        {canEdit && markerPosition(plant) ? <Button size="sm" variant="ghost" onClick={() => onReset(plant.id)}>Reset automatic position</Button> : null}
...
function PlotPlanOverlay({ zones, plants, layouts, viewport, plotId, canEdit, mobile = false, onSelectZone, onMarkerPositionChange, onMarkerPositionReset }) {
  ...
  return (
    <div className={`designer-plan-overlay ${repositioningId ? 'is-marker-repositioning' : ''}`}>
      {repositioningId ? <div className="plot-marker-reposition-notice" role="status">Move the selected marker inside its zone.</div> : null}
      {markers.map((marker) => {
        if (marker.type === 'aggregate') return <button key={marker.id} type="button" className="plot-plant-marker plot-plant-marker--aggregate" style={{ left: marker.x, top: marker.y }} onClick={() => { onSelectZone(marker.zone); setExpandedZoneId(marker.zone.id) }} aria-label={plotPlanText('activePlantings', { count: marker.count })}><PlantVisualIcon plant={{ type: 'leafy', name: 'Crops' }} className="plot-plant-icon" /> {marker.count}</button>
        if (marker.type === 'overflow') return <button key={marker.id} type="button" className="plot-plant-marker plot-plant-marker--overflow" style={{ left: marker.x, top: marker.y }} onClick={() => { onSelectZone(marker.zone); setExpandedZoneId(marker.zone.id) }} aria-label={`Show all crops in ${marker.zone.name}`}>{plotPlanText('morePlantings', { count: marker.count })}</button>
