...
describe('PlotAnalyticsPage', () => {
  ...
  it('keeps generation disabled until at least one analysis type is selected and renders the chosen section', async () => {
    ...
    const generateButton = screen.getByRole('button', { name: /Generate analytics/i })
    ...
    expect(screen.getByRole('heading', { name: /Planning decision analysis/i })).toBeInTheDocument()
  ...
  it('limits analytics header badges to the access role and active plot name', async () => {
    ...
    const headerMeta = screen.getByLabelText(/Plot metadata/i)
    ...
    expect(headerMeta).not.toHaveTextContent('Choose analytics sets')
  })
  ...
  it('renders warnings and no-data sections without crashing', async () => {
    ...
    await user.click(screen.getByLabelText(/Planning decisions/i))
    await user.click(screen.getByLabelText(/Harvest history, quantity trends/i))
    await user.click(screen.getByRole('button', { name: /Generate analytics/i }))
    ...
    expect(screen.getByText(/This plot does not have any harvest history yet/i)).toBeInTheDocument()
  })
})
