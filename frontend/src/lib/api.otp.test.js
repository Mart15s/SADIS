...
describe('OTP CSRF protection', () => {
  ...
  it.each([
    ...
    expect(axiosGet).toHaveBeenCalledWith('/sanctum/csrf-cookie', expect.objectContaining({ withCredentials: true }))
    expect(clientPost).toHaveBeenCalledWith(path, payload)
    expect(axiosGet.mock.invocationCallOrder[0]).toBeLessThan(clientPost.mock.invocationCallOrder[0])
  })
})
